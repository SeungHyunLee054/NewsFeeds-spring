...
public interface FriendshipRepository extends JpaRepository<Friendship, Long> {
	...
	@Query("""
			SELECT new com.nbc.newsfeeds.domain.friend.model.response.FriendRequestResponse(f.id, m.id, m.name)
			FROM Friendship f, Member m
			WHERE f.friendId = :memberId AND f.status = 'PENDING' AND f.id < :cursor AND f.memberId = m.id
			ORDER BY f.id DESC
		""")
